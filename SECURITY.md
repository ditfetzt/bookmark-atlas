...
`data/bookmarks.db`, captured post text, or tokens.
...
- Anyone with read access to your filesystem can read `data/bookmarks.db`. It
  is not encrypted.
...
- `data/` is gitignored for this reason. Do not commit it, and do not point
  `BOOKMARK_ATLAS_DB` at a synced or shared folder unless you accept that.
