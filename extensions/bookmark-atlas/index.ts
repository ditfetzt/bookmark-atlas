...
class BookmarkPalette implements Component, Focusable {
	...
	constructor(items: Bookmark[], theme: Theme, done: (action: Action) => void, initialQuery = "") {
		this.items = items;
		this.theme = theme;
		this.done = done;
	...
	private applyFilter(query: string): Bookmark[] {
		...
		return fuzzyFilter(this.items, query, (bookmark) =>
	...
	render(width: number): string[] {
		...
		lines.push(
			row(
				theme.fg("accent", theme.bold("Bookmark Atlas")) +
					theme.fg("dim", `  ${this.items.length} bookmarks  ·  ${this.filtered.length} shown`),
			),
		);
		...
		lines.push(theme.fg("border", `└${"─".repeat(width - 2)}┘`));
		lines.push(
			theme.fg("dim", "  ↑↓ navigate · enter insert · ctrl+y copy url · ctrl+o open · esc close"),
		);
		return lines;
	}
	...
	pi.registerCommand("bookmarks", {
		...
		handler: async (args: string, ctx: ExtensionCommandContext) => {
			...
			const result = await ctx.ui.custom<Action>(
				(_tui, theme, _keybindings, done) => new BookmarkPalette(items, theme, done, args.trim()),
